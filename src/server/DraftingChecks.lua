...
function Checks.run(g, check, a, b, results)
	...
	local function place(p, pos)
		...
		if not g.clientDiagnostics then task.wait(0.15); return end
		serial += 1
		...
		assert(waitFor(function()
			...
			local root = g:root(p)
			return diag and diag.token == token and diag.passed and root
				and Vector2.new(root.Position.X - pos.X, root.Position.Z - pos.Z).Magnitude <= 3
		end, 5), "Drafting fixture position did not settle")
	end
	...
	local function settleAtHome()
		...
		end, 3), "Drafting cleanup did not leave training")
	end
	...
	check("One trainer remains solo while two adjacent real trainers draft symmetrically", function()
		...
		end, 3), "Solo training incorrectly activated drafting")
		place(b, g.profiles[b].base.treadmill.Position + Vector3.new(0, 3, 0))
		assert(waitFor(function()
			return g.profiles[a].training and g.profiles[b].training
				and g.profiles[a].drafting and g.profiles[b].drafting
				and g.profiles[a].base.treadmill:GetAttribute("Drafting") == true
				and g.profiles[b].base.treadmill:GetAttribute("Drafting") == true
		end, 3), "Adjacent simultaneous trainers did not enter drafting")
		...
		assert(waitFor(function()
			return g.profiles[a].training and not g.profiles[a].drafting and not g.profiles[b].training
		end, 3), "Drafting did not end when the partner stopped training")
		settleAtHome()
	end)
	...
	check("Social Drafting is a non-stacking Speed-gain sidegrade with no tactical or economy authority", function()
		...
		assert(pro.money.Value == money and g.inventory:income(a.UserId) == income
			and pro.lab.momentum == momentum and pro.lab.charge == charge and pro.lab.energy == energy)
		pro.drafting = false
	end)
