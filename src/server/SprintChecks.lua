...
function Checks.run(g, check, a, b, results)
	...
	check("Real two-client Sprint movement completes one normalized race with no economy mutation", function()
		stage()
		local coinsA, coinsB = g.profiles[a].money.Value, g.profiles[b].money.Value
		local idsA, idsB = #g.inventory:list(a.UserId), #g.inventory:list(b.UserId)
		local enteredA = g.profiles[a].lab.records.sprintEntered or 0
		local enteredB = g.profiles[b].lab.records.sprintEntered or 0
		local winsA = g.profiles[a].lab.records.sprintWins or 0
		local winsB = g.profiles[b].lab.records.sprintWins or 0
		assert(g.sprints:request(a, b))
		...
		assert((g.profiles[a].lab.records.sprintEntered or 0) == enteredA + 1)
		assert((g.profiles[b].lab.records.sprintEntered or 0) == enteredB + 1)
		assert(
			(g.profiles[a].lab.records.sprintWins or 0) + (g.profiles[b].lab.records.sprintWins or 0)
				== winsA + winsB + 1
		)
		assert(type(g.profiles[a].lab.records.bestSprint) == "number")
		assert(type(g.profiles[b].lab.records.bestSprint) == "number")
		assert(g.profiles[a].money.Value == coinsA and g.profiles[b].money.Value == coinsB)
		assert(#g.inventory:list(a.UserId) == idsA and #g.inventory:list(b.UserId) == idsB)
	end)
end
